...
2. **AI（Gemini）への送信は最小限に留める。** 現状は検索最適化・食材名寄せ目的でタイトル・食材名のみを送信し、営利目的の学習用途ではない。
   - **送信先は Gemini Developer API（Google AI, `generativelanguage.googleapis.com`）で、現状は無料 tier**。無料 tier は送信データが Google の製品改善・モデル学習に利用されうる（有料 tier は学習非利用）。
   - タイトル・食材名は著作権保護されにくく個人情報も含まないため現状は低リスクと判断し、**無料 tier のまま様子見**とする。
   - ただし **収益化（広告・サブスク等）を行う場合は Kurashiru 利用規約 第16条(17)（営利目的の AI 入力禁止）に抵触しうるため、有料 tier へ切り替える**こと。判断は Issue #132 で追跡。
