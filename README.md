...
LINE で気になったレシピの URL を送るだけで、ページの構造化データ（JSON-LD 等）から「なす」「鶏肉」などのメイン食材を自動で抽出してタグ付け・保存。スーパーや冷蔵庫の前で食材タグをタップすれば、過去に自分が「美味しそう」と思って保存したレシピだけが即座に並びます。

## 主な機能

- **URL からレシピを自動集約** — URL を送ると、ページの構造化データからレシピ名・メイン食材・材料リスト・調理時間を自動抽出してタグ付け
...
| AI | Google Gemini（Vercel AI SDK 経由）— セマンティック検索の埋め込み生成・食材名寄せ辞書の自動生成に利用 |
