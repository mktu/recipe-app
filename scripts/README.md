...
実装後に**自分で動かして確認する**ための土台を1コマンドで作る。
外部サイトへは一切アクセスせず、決め打ちのレシピ4件を DB に入れるだけ。
食材リンク（`recipe_ingredients`）も張るので、ホームのカードが実物どおりに見える。

見た目の確認で詰まりやすいケースを入れてある:

| レシピ | 確認できること |
|---|---|
| 鶏むね肉の照り焼き | 画像あり・食材5件（カードのバッジが `+2` になる） |
| 麻婆豆腐 | 画像なし（🍳 プレースホルダ）・メモ複数行 |
| 休日にまとめて作る…（長いタイトル） | タイトルの折り返し |
| 材料が取れなかったレシピ | 出典なし・材料が空（「材料情報がありません」） |

**前提条件:**
- ローカル Supabase が起動していること (`supabase start`)
- `dev-user-001` の users 行があること（無ければ `npx supabase db reset`）

> **ローカル専用。** `.env.local` が localhost を指していないと実行を拒否する。
>
> **ローカルで E2E を回すと消える。** fixture が `dev-user-001` のレシピを
> 全削除するため（Issue #190）。消えたら再実行する。

解析まで含めて実データで試したい場合は、下の「レシピ登録テスト」を使う。
...
`public/placeholders/<key>.png`（ジャンルの線画アイコン7種＋画像なし用の `default.png`）を
SVG から書き出す。生成物もコミットする。色や形を変えたとき、候補を足したときに実行する。

- 候補の一覧は `src/lib/recipe/placeholder-images.ts`。足したらスクリプトにもアイコンを描く
  （描き忘れは型エラーになる）
- 具体的な料理ではなく、ジャンルの記号にしている。料理の絵だと、別の料理のノートに付けたときに
  タイトルと食い違うため（Issue #174）
- 一覧は画像を正方形に切り抜くので、アイコンは中央の正方形（x=226〜798）の内側に描く
