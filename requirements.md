...
> **注記: 本書は構想時点の要求仕様であり、現在の実装と一部乖離があります。**
>
> 特にレシピ解析については、構想時は LLM による「AI 解析」を想定していましたが、
> **法的リスク（取得元コンテンツの扱い）を踏まえて AI 解析は採用せず**、ページの
> 構造化データ抽出（JSON-LD → `__NEXT_DATA__` → OGP）＋ルールベースの食材マッチング
> に切り替えました。AI（Gemini）は、セマンティック検索の埋め込み生成と食材名寄せ
> 辞書の自動生成にのみ利用しています。
>
> 現在の実装の正本は `docs/ARCHITECTURE.md` を参照してください。
